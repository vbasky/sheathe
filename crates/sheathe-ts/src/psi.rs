...
/// Strip the pointer field (when `payload_unit_start`) and validate the section header.
fn section_body(payload: &[u8], expected_table_id: u8) -> Result<&[u8]> {
    let (body, _) = strip_pointer(payload)?;
    if body.is_empty() {
        return Err(Error::malformed("empty PSI section"));
    }
    if body[0] != expected_table_id {
        return Err(Error::malformed(format!(
            "expected PSI table_id 0x{expected_table_id:02x}, got 0x{:02x}",
            body[0]
        )));
    }
    let syntax = body[1] & 0x80 != 0;
    if !syntax {
        return Err(Error::malformed("PSI section_syntax_indicator not set"));
    }
    let section_length = u16::from(body[1] & 0x0f) << 8 | u16::from(body[2]);
    let total = 3 + usize::from(section_length);
    body.get(..total).ok_or_else(|| Error::malformed("truncated PSI section"))
}
