...
pub(crate) fn pes_sample(data: &[u8], pts: u64, dts: u64, duration: u32) -> Sample {
    let keyframe = split_nals(data)
        .iter()
        .any(|n| n.first().map(|b| b & 0x1f) == Some(AVC_NAL_IDR));
...
pub(crate) fn hevc_parameter_sets(data: &[u8]) -> (Option<Vec<u8>>, Option<Vec<u8>>, Option<Vec<u8>>) {
    let mut vps = None;
    let mut sps = None;
    let mut pps = None;
    for nal in split_nals(data) {
...
pub(crate) fn hevc_pes_sample(data: &[u8], pts: u64, dts: u64, duration: u32) -> Sample {
    let keyframe = split_nals(data).iter().any(|n| {
        matches!(hevc_nal_type(n), HEVC_NAL_IDR_W_RADL | HEVC_NAL_IDR_N_LP)
    });
    let mut flags = SampleFlags::empty();
    if keyframe {
        flags.insert(SampleFlags::KEYFRAME);
