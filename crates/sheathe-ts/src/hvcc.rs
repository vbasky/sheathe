...
pub(crate) fn hvc1_sample_entry(vps: &[u8], sps: &[u8], pps: &[u8], width: u16, height: u16) -> Vec<u8> {
...
fn parse_profile_tier_level(br: &mut BitReader<'_>, max_sub_layers_minus1: u8) -> Option<ProfileTierLevel> {
    ...
    constraint[0] = ((progressive << 7) | (interlaced << 6) | (non_packed << 5) | (frame_only << 4))
        as u8;
