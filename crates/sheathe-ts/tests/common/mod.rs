...
pub fn build_ts_video() -> Vec<u8> {
    let mut out = Vec::new();
    out.extend_from_slice(&ts_packet(0x0000, true, &psi_payload(&pat_section())));
    out.extend_from_slice(&ts_packet(
        PMT_PID,
        true,
        &psi_payload(&pmt_section(&[(0x1b, VIDEO_PID)])),
    ));
...
pub fn build_ts_hevc() -> Vec<u8> {
    ...
    out.extend_from_slice(&ts_packet(
        PMT_PID,
        true,
        &psi_payload(&pmt_section(&[(0x24, VIDEO_PID)])),
    ));
...
pub fn build_ts_aac() -> Vec<u8> {
    ...
    out.extend_from_slice(&ts_packet(
        PMT_PID,
        true,
        &psi_payload(&pmt_section(&[(0x0f, AUDIO_PID)])),
    ));
...
fn pat_section() -> Vec<u8> {
    let mut body = vec![
        0x00, // table_id
        0xb0,
        0x0d, // section_syntax + length placeholder
        0x00,
...
fn pmt_section(streams: &[(u8, u16)]) -> Vec<u8> {
    let mut body = vec![
        ...
        (PMT_PID & 0xff) as u8,
        0xf0,
        0x00, // PCR PID + program_info_length
    ];
...
/// Wrap a PSI section in a TS-packet payload: a `pointer_field` of 0 followed by
/// the section. The demuxer strips the pointer at the transport layer.
